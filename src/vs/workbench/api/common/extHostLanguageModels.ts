...
import { COPILOT_VENDOR_ID, OMENIDE_DEFAULT_VENDOR_ID, IChatMessage, IChatResponsePart, ILanguageModelChatInfoOptions, ILanguageModelChatMetadata, ILanguageModelChatMetadataAndIdentifier, ILanguageModelChatRequestOptions } from '../../contrib/chat/common/languageModels.js';
...
export class ExtHostLanguageModels implements ExtHostLanguageModelsShape {
	...
	async getDefaultLanguageModel(extension: IExtensionDescription, forceResolveModels?: boolean): Promise<vscode.LanguageModelChat | undefined> {
		...
		for (const [modelIdentifier, modelData] of this._localModels) {
			if (modelData.metadata.isDefaultForLocation[ChatAgentLocation.Chat] && (modelData.metadata.vendor === OMENIDE_DEFAULT_VENDOR_ID || modelData.metadata.vendor === COPILOT_VENDOR_ID)) {
