...
You address CodeRabbit (and related) review feedback **and compile/typecheck failures** on an `ai-authored` PR.

Goals:
- Read **unresolved** CodeRabbit review threads (source of truth).
- Fix **omen-typecheck** / TypeScript compile errors when present.
...
- Finishing while `omen-typecheck` is still failing is a **job failure**.
...
- After edits, prefer `npm run typecheck-client` (run `npm ci` first if `node_modules` is missing) and fix every reported TS error before finishing.
- Do not expand scope beyond the review feedback, typecheck errors, and linked issue.
...
When every CodeRabbit thread is fixed (or obsolete), typecheck is clean, and changes are pushed, call `finish_address_review` with `clean=true` and the resolved thread ids.
If still actionable after your fixes, `clean=false` (the job will still fail/retry until threads clear and typecheck is green).
